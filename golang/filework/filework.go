...
// PythonProxy проксирует запросы, отправленные на /api/ к Python-сервису
func PythonProxy(w http.ResponseWriter, r *http.Request) {
	// Получаем исходный путь запроса
	path := r.URL.Path[len("/api/"):]

	// Формируем URL для запроса к Python-сервису
	targetURL := "http://localhost:5000/" + path

	// Создаем новый запрос к Python-сервису
	...
	// Копируем статус ответа от Python-сервиса
